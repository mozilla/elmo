...
class StatusJSON(SignoffDataView):
    ...
    def get_signoffs(self):
        ...
        for action, signoff, push_date in (
                Action.objects
        ...
        runs_with_open_av = [
            run for run, tree in self.runids2tree.items()
            if tree in tree_ids
        ...
        for (treeid, locale_code), changeset in six.iteritems(changesets):
            ...
            if avt.end and push_date > avt.end:
                continue
            if (
                    (avt.appversion, locale_code) in last_action and
                    last_action[(avt.appversion, locale_code)] >= push_date
            ):
                continue
            maybe_new_run.append((
                avt.appversion, changeset, avt.tree.code,

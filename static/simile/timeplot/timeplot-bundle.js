...
},paint:function(){var O=this._canvas.getContext("2d");
O.lineWidth=this._plotInfo.lineWidth;
O.lineJoin="miter";
var K=this._plotInfo.flat;
if(this._dataSource){if(this._plotInfo.fillColor){if(this._plotInfo.fillGradient){var A=O.createLinearGradient(0,this._canvas.height,0,0);
...
O.fillStyle=A;
}else{O.fillStyle=this._plotInfo.fillColor.toString();
}O.beginPath();
O.moveTo(0,0);
var C=0;
this._plot(function(V,W){if(K){O.lineTo(V,C);
C=W;
}O.lineTo(V,W);
});
if(this._plotInfo.fillFrom==Number.NEGATIVE_INFINITY){O.lineTo(this._canvas.width,0);
}else{if(this._plotInfo.fillFrom==Number.POSITIVE_INFINITY){O.lineTo(this._canvas.width,this._canvas.height);
O.lineTo(0,this._canvas.height);
}else{O.lineTo(this._canvas.width,this._valueGeometry.toScreen(this._plotInfo.fillFrom));
O.lineTo(0,this._valueGeometry.toScreen(this._plotInfo.fillFrom));
}}O.fill();
}if(this._plotInfo.lineColor){O.strokeStyle=this._plotInfo.lineColor.toString();
O.beginPath();
var G=true;
var C;
this._plot(function(V,W){if(G){G=false;
O.moveTo(V,W);
C=W;
}if(K){O.lineTo(V,C);
C=W;
...
O.stroke();
}if(this._plotInfo.dotColor){O.fillStyle=this._plotInfo.dotColor.toString();
var M=this._plotInfo.dotRadius;
this._plot(function(V,W){O.beginPath();
O.arc(V,W,M,0,2*Math.PI,true);
O.fill();
});
}}if(this._eventSource){var A=O.createLinearGradient(0,0,0,this._canvas.height);
A.addColorStop(1,"rgba(255,255,255,0)");
O.strokeStyle=A;
O.fillStyle=A;
O.lineWidth=this._plotInfo.eventLineWidth;
O.lineJoin="miter";
var S=this._eventSource.getAllEventIterator();
while(S.hasNext()){var R=S.next();
var P=R.getColor();
P=(P)?new Timeplot.Color(P):this._plotInfo.lineColor;
var B=R.getStart().getTime();
var T=R.getEnd().getTime();
if(B==T){var U=P.toString();
A.addColorStop(0,U);
var F=this._timeGeometry.toScreen(B);
F=Math.floor(F)+0.5;
var D=F;
O.beginPath();
O.moveTo(F,0);
O.lineTo(F,this._canvas.height);
O.stroke();
var I=F-4;
var J=7;
}else{var U=P.toString(0.5);
A.addColorStop(0,U);
var F=this._timeGeometry.toScreen(B);
F=Math.floor(F)+0.5;
var D=this._timeGeometry.toScreen(T);
D=Math.floor(D)+0.5;
O.fillRect(F,0,D-F,this._canvas.height);
var I=F;
var J=D-F-1;
}var L=this._timeplot.putDiv(R.getID(),"timeplot-event-box",{left:Math.round(I),width:Math.round(J),top:0,height:this._canvas.height-1});
var Q=this;
var H=function(V){return function(Y,W,b){var a=Q._timeplot.getDocument();
Q._closeBubble();
var Z=SimileAjax.DOM.getEventPageCoordinates(W);
var X=SimileAjax.DOM.getPageCoordinates(Y);
Q._bubble=SimileAjax.Graphics.createBubbleForPoint(Z.x,X.top+Q._canvas.height,Q._plotInfo.bubbleWidth,Q._plotInfo.bubbleHeight,"bottom");
V.fillInfoBubble(Q._bubble.content,Q._theme,Q._timeGeometry.getLabeler());
};
};
var E=function(W,V,X){W.oldClass=W.className;
W.className=W.className+" timeplot-event-box-highlight";
};
var N=function(W,V,X){W.className=W.oldClass;
W.oldClass=null;
};
if(!L.instrumented){SimileAjax.DOM.registerEvent(L,"click",H(R));
SimileAjax.DOM.registerEvent(L,"mouseover",E);
SimileAjax.DOM.registerEvent(L,"mouseout",N);
L.instrumented=true;
